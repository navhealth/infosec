...
1. Requests for access to CareJourney Platform systems and applications are made formally using the following process:
  1. A CareJourney workforce member initiates the access request by creating an Issue in the CareJourney Quality Management System.
...
4. All access to CareJourney systems and services is reviewed and updated on a bi-annual basis to ensure proper authorizations are in place commensurate with job functions. The process for conducting reviews is outlined below:
   1. The Security Officer initiates the review of user access by creating an Issue in the CareJourney Quality Management System.
   2. The Security Officer is assigned to review levels of access for each CareJourney workforce member.
...
5. Any CareJourney workforce member can request change of access using the process outlined in [§7.2 paragraph 1](#7.2-access-establishment-and-modification).
...
8. In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of CareJourney Customers to define, and not CareJourney.
...
11. Generic accounts are not allowed on CareJourney systems.
...
3. CareJourney maintains a minimum necessary approach to access to Customer data. As such, CareJourney, including all workforce members, does not readily have access to any ePHI.

## 7.4 Access Authorization

1. Role based access categories for each CareJourney system and application are pre-approved by the Security Officer, or an authorized delegate of the Security Officer.
2. CareJourney utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
3. All Customer support desk interactions must be verified before CareJourney support personnel will satisfy any request having information security implications.
   * CareJourney's current support desk software, Zendesk, requires users to authenticate before submitting support tickets.
   * Support issues submitted via CareJourney's dashboard require that users authenticate with their CareJourney account before submitting support tickets.
   * Support issues submitted by email must be verified by CareJourney personnel using a phone number that has been registered with the corresponding account.

## 7.6 Unique User Identification

1. Access to the CareJourney Platform systems and applications is controlled by requiring unique User Login IDs and passwords for each individual user and developer.
2. Passwords requirements mandate strong password controls (see below).
3. Passwords are not displayed at any time and are not transmitted or stored in plain text.
4. Default accounts on all production systems, including root, are disabled.
5. Shared accounts are not allowed within CareJourney systems or networks.
6. Automated log-on configurations that store user passwords or bypass password entry are not permitted for use with CareJourney workstations or production systems.
...
1. CareJourney production systems are not accessible directly over wireless channels.
...
4. Wireless networks managed within CareJourney non-production facilities (offices, etc.) are secured with the following configurations:
   * All data in transit over wireless is encrypted using WPA2 encryption;
   * Passwords are rotated on a regular basis, presently quarterly. This process is managed by the CareJourney Security Officer.
...
CareJourney does not use paper records for any sensitive information. Use of paper for recording and storing sensitive data is against CareJourney policies.
...
2. Disallowing transfer of ePHI to workstations is enforced through technical measures.
   ...
   * Configuration settings for enforcing these technical controls are managed by CareJourney's configuration management tooling, Chef/Salt.

## 7.14 PaaS Customer Access to Systems

CareJourney grants PaaS customer secure system access via VPN connections. This access is only to Customer-specific systems, no other systems in the environment. These connections are setup at customer deployment. These connections are secured and encrypted and the only method for customers to connect to CareJourney hosted systems.

In the case of data migration, CareJourney does, on a case by case basis, support customers in importing data. In these cases CareJourney requires that all data is secured and encrypted in transit, such as by using SFTP or SCP for transferring files.

In the case of an investigation, CareJourney will assist customers, at CareJourney's discretion, and law enforcement in forensics.
